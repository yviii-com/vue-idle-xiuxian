...
const createPlayerEntity = () => {
  ...
  const baseStats = {
    ...
    // 增益效果相关
    buffEffectiveness: playerStore.buffEffectiveness || 1
  }

  // 应用所有活跃的增益效果
  activeBuffs.forEach(buff => {
    if (typeof buff.effect === 'function') {
      buff.effect(baseStats)
    }
  })
      ...
      <n-space vertical>
        ...
        <template v-if="dungeonState.inCombat && dungeonState.combatManager">
          <n-card :title="`战斗信息(${dungeonState.combatManager.round} / ${dungeonState.combatManager.maxRounds})`">
            <!-- 玩家属性 -->
            <n-descriptions bordered :title="`${dungeonState.combatManager.player.name}的属性`" :column="2">
            ...
            <!-- 敌人属性 -->
            <n-descriptions bordered :title="`${dungeonState.combatManager.enemy.name}的属性`" :span="2"
