...
onMounted(() => {
  // 确保玩家数据已初始化
  if (!playerStore.completedAchievements) {
    playerStore.completedAchievements = []
  }
  // 检查成就完成情况，只显示新完成的成就
  const newlyCompletedAchievements = checkAchievements(playerStore).filter(achievement => {
    // 过滤掉在本次检查之前就已经完成的成就
    return !playerStore.completedAchievements.includes(achievement.id)
  })
  // 显示新完成的成就
...
function getProgress (achievement) {
  try {
    ...
    if (!Number.isFinite(progress)) {
      console.error('Invalid progress value for achievement:', achievement.id, progress)
      return 0
    }
    return Math.min(100, Math.max(0, Math.round(progress)))
  } catch (error) {
    console.error('成就进度计算错误:', error, achievement)
    return 0
  }
}
</script>
