...
<template>
    ...
    <template v-if="selectedPet">
      ...
      <n-collapse>
        <n-collapse-item title="展开" name="1">
          <n-descriptions bordered title="基础信息" :column="2">
            <n-descriptions-item
              label="攻击力">{{ (selectedPet.combatAttributes?.attack || 0).toFixed(0) }}</n-descriptions-item>
            <n-descriptions-item
              label="生命值">{{ (selectedPet.combatAttributes?.health || 0).toFixed(0) }}</n-descriptions-item>
            <n-descriptions-item
              label="防御力">{{ (selectedPet.combatAttributes?.defense || 0).toFixed(0) }}</n-descriptions-item>
            <n-descriptions-item
              label="速度">{{ (selectedPet.combatAttributes?.speed || 0).toFixed(0) }}</n-descriptions-item>
          </n-descriptions>
          <n-descriptions bordered title="战斗信息" :column="3">
            <n-descriptions-item
  ...
  <n-modal v-model:show="showEquipmentDetailModal" preset="dialog" :title="selectedEquipment?.name || '装备详情'">
    ...
    <div v-if="equipmentComparison" class="stats-comparison">
      <h4>属性对比</h4>
      <n-table :bordered="false" :single-line="false">
        ...
        <tbody>
          <tr v-for="(comparison, stat) in equipmentComparison" :key="stat">
            ...
            <td>
              <span :class="{ 'positive': comparison.isPositive, 'negative': !comparison.isPositive }">
