...
// 辅助函数
const helpers = {
  getTotalEquipment: (player) => {
    const equippedCount = Object.values(player.equippedArtifacts || {}).filter(
      (e) => e !== null
    ).length;
    const inventoryCount = (player.equipment || []).length;
    return equippedCount + inventoryCount;
  },
  hasEquipmentWithQuality: (player, quality) => {
    return (
      Object.values(player.equippedArtifacts || {}).some(
        (e) => e?.quality === quality
      ) || (player.equipment || []).some((e) => e?.quality === quality)
    );
  },
  getMaxEnhanceLevel: (player) => {
    return Math.max(
      0,
      ...Object.values(player.equippedArtifacts || {})
        .filter((e) => e !== null)
        .map((e) => e?.enhanceLevel || 0),
      ...(player.equipment || []).map((e) => e?.enhanceLevel || 0)
    );
  },
  countEquipmentTypes: (player, quality) => {
    const allEquipment = [
      ...Object.values(player.equippedArtifacts).filter((e) => e !== null),
      ...(player.equipment || []),
    ];
    return new Set(
      allEquipment.filter((e) => e.quality === quality).map((e) => e.type)
    ).size;
  },
};
...
export const achievements = {
  ...
  equipment: [
    ...
    },
    {
      ...
      condition: (player) => {
        const equippedLegendary = Object.values(player.equippedArtifacts).some(
          (e) => e?.quality === "legendary"
        );
        const inventoryLegendary = player.equipment?.some(
          (e) => e.quality === "legendary"
        );
    ...
    },
    {
      ...
      condition: (player) => {
        const equippedEnhanced = Object.values(player.equippedArtifacts).some(
          (e) => e?.enhanceLevel >= 10
        );
        const inventoryEnhanced = player.equipment?.some(
          (e) => e.enhanceLevel >= 10
        );
    ...
    },
    {
      ...
      condition: (player) => {
        ...
        const legendaryTypes = new Set(
          allEquipment
            .filter((e) => e.quality === "legendary")
            .map((e) => e.type)
        );
    ...
    },
    {
      ...
      condition: (player) => {
        const equippedMythic = Object.values(player.equippedArtifacts).filter(
          (e) => e?.quality === "mythic"
        ).length;
        const inventoryMythic = (player.equipment || []).filter(
          (e) => e.quality === "mythic"
        ).length;
    ...
    },
    {
      ...
      condition: (player) => {
        ...
        const mythicTypes = new Set(
          allEquipment.filter((e) => e.quality === "mythic").map((e) => e.type)
        );
  ...
  collection: [
    ...
    },
    {
      ...
      condition: (player) =>
        player.herbs.filter((h) => h.quality === "rare").length >= 100,
      reward: { spirit: 2000, herbRate: 1 },
    },
    {
      id: "collection_8",
      ...
      condition: (player) =>
        player.herbs.filter((h) => h.quality === "epic").length >= 100,
    ...
    },
    {
      ...
      condition: (player) =>
        player.herbs.filter((h) => h.quality === "mythic").length >= 100,
...
export const checkAchievements = (player) => {
  // 初始化玩家属性
  Object.entries(defaultPlayerStats).forEach(([key, value]) => {
...
export const getAchievementProgress = (player, achievement) => {
  ...
  // 使用achievement.target来计算进度
  const current = achievement.getCurrentValue?.(player) ?? 0;
